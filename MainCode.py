...
def main():
    ...
    if choice == menu[0]:
        ...
        Beta1=st.number_input("Insert the shape parameter of the weak component (β\u2082)", min_value = 0.0, value = 3.0)
        Eta2=st.number_input("Insert the characteristic life of the strong component (η\u2081)", min_value = 3.0, value = 12.0)
        ...
        FixedCosts=st.text_input("Insert the fixed costs of the inspectors ($C^H$)", value = "0.1 0.1")
        Ci=st.text_input("Insert the inspection costs of the inspectors ($C^I$)", value = "0.1 0.15")
        ...
        Cr=st.number_input("Insert cost of replacement (inspections and age-based) ($C^R$)", min_value = 0.5, value = 1.0)
        Cf=st.number_input("Insert cost of failure ($C^F$)", min_value = 5, value = 10)
    ...
    if choice == menu[1]:
        ...
        st.write("<h6 style='text-align: justify; color: Blue Jay;'>The app computes the cost rate for a specific solution—defined by inspection intervals (Δ), inspector assignments (Y), and the age-based threshold (T). It is crucial that Δ and Y have the same number of elements and that T is greater than or equal to the last inspection moment. For further questions or information on finding the optimal solution, please contact one of the email addresses below.</h6>", unsafe_allow_html=True)
